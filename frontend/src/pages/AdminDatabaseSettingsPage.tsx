...
import { Database, RefreshCw, Trash2, Download, AlertCircle, CheckCircle, Clock, HardDrive, BarChart3, Users, Building, Calendar } from 'lucide-react';
...
export default function AdminDatabaseSettingsPage() {
  console.log('AdminDatabaseSettingsPage component rendering');
  ...
  const fetchStats = async () => {
    try {
      ...
      toast.error('Failed to fetch database stats: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Content - Only show when stats are loaded */}
      {stats && (
        <>
          {/* Health Status */}
          {health && (
